...
- 타입 라벨: 현재 프로토타입 설명용. 실제 인게임에서는 가능한 한 아이콘/상태만으로 읽히게 하고 텍스트는 제거하거나 디버그용으로 둔다.
...
- count label: 프로토타입에서는 `2/3` 형태로 하단 표시, 실제 인게임에서는 모바일 가독성을 보고 유지 여부 결정
...
- 내부 슬롯: 원형 슬롯 3개
- 슬롯 예시 위치: `(-0.26, 0.20)`, `(-0.28, -0.16)`, `(0.27, -0.03)`
- 중앙 허브: 원형 activation light, 반지름 `0.12`
- 슬롯과 허브는 얇은 선으로 연결한다.
- 연결선 두께: `0.04`
- 연결선 색: 리소스 색 alpha `0.62`
- 활성 전: 중앙 허브는 red
- 모든 슬롯 충족 후: 중앙 허브는 green
