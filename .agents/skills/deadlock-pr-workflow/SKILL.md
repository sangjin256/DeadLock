...
- `ADD`: 에셋, 패키지, 이미지, 사운드처럼 단순 파일 추가.
- `FEAT`: 새로운 기능 구현 또는 사용자에게 보이는 동작 추가.
- `FIX`: 버그 수정.
- `STYLE`: 코드 수정 없이 오브젝트나 컴포넌트 속성값 변경.
- `REFACTOR`: 기능 변화 없는 코드 리팩터링 또는 구조 개선.
- `DOCS`: 문서, 폴더 정리, 파일/폴더명 수정 또는 이동.
- `CHORE`: 프로젝트 설정, 빌드 설정, 패키지 설정, 스킬, 자동화 같은 관리 작업.
- `REMOVE`: 불필요한 파일 삭제.
- `TEST`: 테스트 추가 또는 수정.

이미 좋은 커밋 메시지나 브랜치명이 있으면 참고하되, 최종 PR 제목은 위 형식을 따른다.

제목은 50자 이내로 간결하게 작성하고, 마침표와 불필요한 특수기호를 붙이지 않는다. 현재시제를 사용한다.

## 브랜치 정책

- 브랜치는 작업 단위로 생성한다.
- 사용자가 별도 형식을 요청하지 않았고 새 브랜치를 제안해야 한다면 `tag-task-name-user` 형식을 사용한다.
- `tag`는 소문자로 쓴다. 예: `feat-level-progress-sangjin`, `docs-update-convention-sangjin`.
- Main 또는 타겟 브랜치에 병합된 브랜치와 동일한 로컬 브랜치를 계속 사용하지 않는다.
- Codex 앱에서 직접 브랜치를 만들 때는 앱 지침에 따라 기본 prefix `codex/`를 붙이되, 브랜치 본문은 프로젝트 브랜치 정책을 따른다. 예: `codex/docs-update-convention-sangjin`.
...
필요한 경우 본문에 변경 이유와 리뷰 참고사항을 적는다.
```

커밋 제목 규칙:

- `TAG`는 PR 제목의 TYPE 기준과 동일하게 고른다.
- 제목은 50자 이내를 목표로 한다.
- 마침표와 불필요한 특수기호를 붙이지 않는다.
- 현재시제와 간결한 표현을 사용한다.
- 본문은 선택사항이지만, 변경 이유가 PR 리뷰에 도움이 되면 자세히 쓴다.
